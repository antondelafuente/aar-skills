...
`log-experiment.sh <registry-dir> [--dry-run]` — classify → gate → branch → PR → approve (cross-family bot) → merge → sync. Config via env (instance, not hardcoded): `RESEARCH_REPO` (the research repo, default `antondelafuente/research-lab`), and the engineer-bot token command (reuse the existing mint seam). `--dry-run` classifies + gates and stops before any push.

## Gate detail (fail-closed)

- **experiment:** BLOCK unless `AUDIT.md` present AND no unresolved HIGH in `AUDIT_RESPONSE.md`. An eval-only / anchor-failed run that legitimately has no close-audit is allowed only if `RESULTS.md` records a closed decision (e.g. `ANCHOR_FAILED`, no-go) — otherwise BLOCK and surface.
