...
- **Track 1 first** — it fixes the staleness biting the fleet today and is small. FU-1 (Claude) and FU-2
  (Codex) are independent and parallel; FU-3 (fleet-update command) composes them.
...
- **FU-3 — `update-fleet` command.** One shot: `git pull` in `~/aar-skills` → broadcast `/reload-plugins`
  to live `claude-N` sessions (via the message-aar/tmux path). Makes "propagate to everyone" one action.

**Track 2 (later):**
- **FU-4 — Auto-bump plugin version on merge.** In the `ship-change`/`wf.sh finish` path, auto-increment the
  changed plugin(s) `plugin.json` version so the marketplace cache key changes and consumers' `plugin update`
  actually refreshes. Defines *when* to bump (behavior change vs any change).
...
and the follow-up issues (FU-1…FU-5) have been **created** from the accepted design. The first proposal
existing is not done.
