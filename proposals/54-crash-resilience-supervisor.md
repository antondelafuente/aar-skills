...
- **create (at acquire).** `gpu-job` acquire writes a **complete** lease record atomically as part of returning pod-success, before the agent does anything else. Required fields: **pod id; deploying API-key name; owner/run artifact path; SSH endpoint; expiry / cost cap**. **Acquire fails loudly (and tears the pod down) if it cannot write a complete lease** — a pod must never exist without a lease, or the reaper's "report unknown, never delete" rule would let it bill forever.
- **refresh (during the run).** A long run extends its `expiry` — the controller-side generalization of today's pod-side `keepalive_until_utc` refresh (lifted off the pod so it survives the pod and the agent). The standing handoff (A) records the active expiry so a successor keeps refreshing.
...
Canonical home: a `gpu-job` lease registry — generic schema + the create/refresh/close helpers in `plugins/gpu-job/skills/gpu-job/scripts/`, instance registry path under `~/.config/gpu-job/`. This fixes the "agent died before arming the watchdog" hole (the lease is written at acquire, not later by the agent) and gives the sweep a complete, fail-closed authority to act on.
...
**Reliable now (the `ready` first cut):** the two unambiguous failure signatures —

- **Process exit / crash.** A non-zero `claude` exit (already what `claude-pane-loop.sh` catches) — extended only so the supervisor *also* covers the cases the in-pane loop can't: the whole tmux session/pane gone, or the box-level wrapper itself dead.
- **Explicit agent-declared stuck-or-blocked.** The agent (or a `StopFailure` hook) writes a small on-disk *needs-relaunch* marker — a positive, model-free signal that recovery is wanted (the policy-block / can't-resume-in-place case), which the supervisor acts on by the fresh-successor path.

Both feed the same relaunch decision: prefer same-session `--continue` (the path `claude-pane-loop.sh` owns), else a **fresh successor via `relaunch-session.sh <name> <dir> handoff`** (the existing seed primitive that points the successor at the standing `TEMP.md`) when same-session resume is impossible (corrupted JSONL, policy block). It carries a **single-writer kill/resume lock** so it never double-relaunches or races the in-pane loop, is **idempotent**, and keeps the loop's crash-storm cap.
...
**Canonical-home decision (fixed here, not deferred).** The **lease schema + lifecycle API + reaper decision logic** are `gpu-job` (pod lifecycle is its domain). The **relaunch decision tree + the standing-handoff contract** are an `experiment-lifecycle` / `run-experiment` reference (executor liveness is that layer's domain). All *instance values* (session names, paths, RunPod key, systemd unit, `claude-pane-loop.sh`, `relaunch-session.sh`) stay under `~/.config/` / instance scripts per the releasability rule. The `ready` children below are `ready` because every product interface boundary they need is named here; the one genuinely-unsolved piece (silent-wedge detection) is split out as a `needs-design` child, not smuggled into a `ready` one.
...
3. **Relaunch supervisor — crash/exit + explicit-marker first cut** (B.2 reliable half) — relaunch on a `claude` exit the in-pane loop can't catch (session/pane/wrapper dead) or an agent-written *needs-relaunch* marker; the decision tree (`--continue` else `relaunch-session.sh handoff`) + single-writer lock + idempotence. Generic decision logic as an `experiment-lifecycle` reference; the systemd/bash wiring is instance. **Does NOT include silent-wedge detection.**
