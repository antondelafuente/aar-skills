...
- A `StopFailure`-style hook (an instance may wire one) can **fire** on an API-error exit — push-notify and/or drop the supervisor's *needs-relaunch* marker — but it **cannot resume** the session itself, because it runs only after the agent process has exited. Recovery is the model-free supervisor's job (`resume_same_session` / `launch_successor`), not the hook's. The hook is a **signal into** that path, never a substitute for it.
...
**Instance side (NOT this repo — done same-day with a pointer back here):** set `API_TIMEOUT_MS` (raised) and `API_FORCE_IDLE_TIMEOUT="0"` in the box's `settings.json`, and add the `StopFailure` hook that push-notifies and writes the child-3 *needs-relaunch* marker. These are inert without the supervisor (children 2/3) consuming the marker, and the settings bump stands alone (it just makes the in-process retry ride out a longer blip).
