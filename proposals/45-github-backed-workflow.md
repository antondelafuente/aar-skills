# Proposal: GitHub-backed design + PR review workflow for AAR scaffold changes

> Design-of-the-change + ADR + PR description. Reviewed by `--scaffold` before build.

> **[Recorded 2026-06-17 — founding ADR (#45).]** This is the original design record for the ship-change /
> aar-engineering workflow, written during its design and only now saved to `main` for the archive. It is a
> **point-in-time ADR**: some elements were *planned but built differently*. As-built differences — the
> `design-gate` classifier is **advisory** (recorded on the PR), not a required status check; the
> `claude-engineer` App and the Codex→Claude reverse review path are **not yet built** (today only
> `codex-engineer` reviews Claude-authored changes); required status checks are **deliberately not enabled**.
> For what actually shipped and the live enforcement config, see the ship-change skill's `RUNBOOK.md` +
> `SKILL.md`. The value preserved here is the *why* — the design rationale and the two folded review rounds.
